...
class DashboardController extends Controller
{
    ...
    */
    ...
    public function downloadQR(Request $request){
        ...
        $url="http://costjalisco.org.mx/project-single/".$request->id_project;
      
        $src="https://chart.googleapis.com/chart?chs=".$dimensiones."&cht=qr&chl=".$url."&choe=UTF-8";

        header('Content-Type: image/png');
header('Content-Disposition: attachment; filename=QR'.$request->id_project.'.png');
...
echo $image;
    ...
    public function downloadLona(Request $request){
        $id=$_GET['id'];

        $project=Project::find($id);

        $org=DB::table('orglogos')
        ->where('id_organization','=',$project->publicAuthority_id)
        ->first();
        
        $ruta="http://costjalisco.org.mx/orglogos/".$org->imgroute;
        //$ruta="http://costjalisco.org.mx/orglogos/1611602581guadalajara.png";


        $titulo=$project->title;
        
        return view('admin/downloadableLona',['project'=>$project,"lng"=>strlen($titulo),'ruta'=>$ruta]);
    }
