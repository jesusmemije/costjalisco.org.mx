        ...
        if($proyecto!=$project->id_project){
            ...
            if ($tipocontrato == null) {
                ...
                array_push($contratos,
                     ...
                     $aux->id,
                     $aux->entidadadjudicacion,
                     $aux->nombrecontacto,
                     $aux->emailcontacto,
                     $aux->telefonocontacto,
                     $aux->domiciliocontacto,
                     $aux->fechapublicacion,
                     $aux->nombreresponsable,
                     $modalidadadjudicacion->titulo,
                     $tipocontrato->titulo,
                     $modalidadcontrato->titulo,
                     $estadoactual->titulo,
                     $aux->empresasparticipantes,
                     $aux->entidad_admin_contrato,
                     $aux->titulocontrato,
                     $aux->empresacontratada,
                     $aux->viapropuesta,
                     $aux->fechapresentacionpropuesta,
                     $aux->montocontrato,
                     $aux->alcancecontrato,
                     $aux->fechainiciocontrato,
                     $aux->duracionproyecto_contrato,
                     $aux->observaciones      
            ...
            foreach($aux_finalizacion as $aux){
                array_push($finalizaciones,
                $aux->id,
                $aux->id_contrato,
                $aux->costofinalizacion,
                $aux->fechafinalizacion,
                $aux->alcancefinalizacion,
                $aux->razonescambioproyecto,
                $aux->referenciainforme,
                $aux->observaciones
                );
          
            }
    
            $finalizaciones=implode("|",$finalizaciones);
